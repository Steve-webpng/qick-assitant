...
const responseSchema = {
  ...
  properties: {
    ...
    viabilityJustification: {
      type: Type.OBJECT,
      description: "A detailed justification for the viability score.",
      properties: {
        summary: { type: Type.STRING, description: "A concise summary explaining the reasoning behind the viability score, linking it to market conditions and SWOT." },
        positiveFactors: { type: Type.ARRAY, items: { type: Type.STRING }, description: "A list of 2-3 key positive factors supporting the idea's viability score." },
        negativeFactors: { type: Type.ARRAY, items: { type: Type.STRING }, description: "A list of 2-3 key risks or considerations that temper the viability score." }
      },
...
export const validateBusinessIdea = async (idea: Idea): Promise<AnalysisReport> => {
  const prompt = `
    ...
    - An honest, data-driven (even if simulated) viability score and a detailed justification. The justification must be an object containing a summary, key positive factors, and key negative factors/risks.
